...
from network_helpers import load_network, get_stochastic_network_move, save_network
    ...
    with tf.Session() as session:
        ...
        for episode_number in range(1, number_of_games):
            ...
            if (not randomize_first_player) or bool(random.getrandbits(1)):
                reward,info = game_spec.play_game(make_training_move, opponent_func, log=False)
            else:
                reward,info = game_spec.play_game(opponent_func, make_training_move, log=False)
                reward = -reward
            results.append(reward)
            mini_batch_illegal_moves+=info['illegal_move']
